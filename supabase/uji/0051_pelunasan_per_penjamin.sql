...
begin
  ...
  v_trx := public.apply_transaction(
    ...
    10000, 'Tunai', '{}'::jsonb, v_co, 'umum', null, 0);
  ...
  v_trx := public.apply_transaction(
    ...
    0, 'Tunai', '{}'::jsonb, v_co, 'bpjs', null, 20000);
  select * into v_row from public.transactions where id = (v_trx ->> 'id')::uuid;
  ...
  v_trx := public.apply_transaction(
    ...
    15000, 'Tunai', '{}'::jsonb, v_co, 'asuransi', v_as, 35000);
  select * into v_row from public.transactions where id = (v_trx ->> 'id')::uuid;
  ...
  v_lap := public.laporan_penjamin(current_date, current_date);
