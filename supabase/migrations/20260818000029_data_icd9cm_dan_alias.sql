-- ============================================================
-- 0029  Isi ICD-9-CM, dan alias Indonesia untuk ICD-10
-- ============================================================
--
-- Dua isi dalam satu berkas karena keduanya kecil dan keduanya hanya masuk
-- akal setelah 0026 sampai 0028 selesai: aliasnya menunjuk ke `icd10(kode)`
-- lewat foreign key, jadi ia akan gagal kalau tabelnya masih kosong. Itu
-- disengaja. Gagal di sini berarti ada bagian ICD-10 yang belum dijalankan,
-- dan itu jauh lebih baik ketahuan sekarang daripada nanti sebagai kotak
-- pencarian yang diam saja.
--
-- 4626 kode tindakan ICD-9-CM versi ICD9CM_2010.

insert into public.icd9cm (kode, nama)
select split_part(x, '|', 1), split_part(x, '|', 2)
from unnest(string_to_array($ICD$00|Procedures and interventions, Not Elsewhere Classified
...
99.99|Other miscellaneous procedures$ICD$, E'\n')) as x
where x <> ''
...
-- Alias Indonesia
-- ------------------------------------------------------------
-- 49 nama yang sebelumnya jadi SELURUH daftar diagnosis aplikasi ini,
-- di `lib/icd10.ts`. Sekarang tugasnya lebih kecil dan lebih jujur: bukan
-- lagi daftar yang boleh dipilih, melainkan jalan pintas mengetik menuju
-- kode resmi. Semuanya sudah dicocokkan, semua 49 kodenya ada di
-- berkas Kemenkes.
--
-- Menambah alias baru aman: kuncinya (kode, nama), jadi satu kode boleh
-- punya beberapa nama Indonesia.

insert into public.icd10_alias (kode, nama)
select split_part(x, '|', 1), split_part(x, '|', 2)
from unnest(string_to_array($ALIAS$J00|Nasofaringitis akut (pilek)
...
F41.9|Gangguan cemas
T14.1|Luka terbuka
Z00.0|Pemeriksaan kesehatan umum$ALIAS$, E'\n')) as x
where x <> ''
on conflict (kode, nama) do nothing;
